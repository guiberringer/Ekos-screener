...
function buildProfileExtractionPrompt(businessName, notes) {
  ...
  const shape = Object.fromEntries(PROFILE_FIELDS.map((f) => [f.key, { value: "unknown", confidence: "unknown", rationale: "one short sentence, under 20 words" }]));

  return `Research notes on "${businessName}":
${notes}

Using ONLY the notes above (there is no web search available in this step — work from the notes alone), fill in these profile fields as best you can. If the notes don't cover something, answer "unknown" — do not invent specifics.
...
function buildPillarExtractionPrompt(businessName, pillar, notes) {
  const pillarLines = pillar.questions.map((q) => `- ${q.id}: ${q.text} Allowed values: ${allowedValuesFor(q.type)}.`).join("\n");
  const shape = Object.fromEntries(pillar.questions.map((q) => [q.id, { value: "unknown", rationale: "one short sentence, under 20 words" }]));

  return `Research notes on "${businessName}":
${notes}

Using ONLY the notes above (there is no web search available in this step — work from the notes alone), answer these questions for the "${pillar.label}" section. If the notes don't cover something, answer "unknown" — do not invent specifics. On nature-dependency style questions: trace materials back to their origin (e.g. paper depends on forestry) rather than stopping at "these are manufactured goods."
...
export default function EkosScreener() {
  ...
  async function runResearch() {
    ...
    try {
      ...
      const profilePromise = callClaudeRaw({
        model: "claude-sonnet-4-6",
        max_tokens: 800,
        messages: [{ role: "user", content: buildProfileExtractionPrompt(businessName, notes) }],
      });
      const pillarPromises = PILLARS.map((p) =>
        callClaudeRaw({
          model: "claude-sonnet-4-6",
          max_tokens: 600,
          messages: [{ role: "user", content: buildPillarExtractionPrompt(businessName, p, notes) }],
        })
      );
      const [profileResult, ...pillarResults] = await Promise.all([profilePromise, ...pillarPromises]);

      const debugParts = [
        `=== PHASE 1: research notes ===\n${notesResult.raw}`,
        `\n\n=== PHASE 2a: profile ===\n${profileResult.raw}`,
      ];
      PILLARS.forEach((p, i) => debugParts.push(`\n\n=== PHASE 2b: ${p.label} ===\n${pillarResults[i].raw}`));
      ...
      if (profileResult.ok) {
        PROFILE_FIELDS.forEach((f) => {
          const r = profileResult.parsed?.[f.key];
      ...
      PILLARS.forEach((p, i) => {
        ...
        const pr = pillarResults[i];
        if (pr.ok) {
          p.questions.forEach((q) => {
            const r = pr.parsed?.[q.id];
            if (r && r.value && r.value !== "unknown") {
  ...
  function renderIntro() {
    return (
      <div className="panel panel--intro">
        <p className="eyebrow">Sustainability screener</p>
  ...
  return (
    <div className="app">
      ...
      <div className="logo-header">
        <img src="/ekos-logo.png" alt="Ekos" />
      </div>
